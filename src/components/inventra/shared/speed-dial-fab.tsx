...
export default function SpeedDialFAB() {
  ...
  // Actions: bottom-to-top visually. Index 0 is closest to FAB.
  ...
  return (
    <>
      ...
      {/* ── Speed Dial Actions — Vertical Stack ── */}
      {mounted && (
        <div
          className="fixed z-[56] right-4 flex flex-col-reverse items-end gap-2.5 lg:hidden"
          style={{ bottom: '88px' }}
        >
          {actions.map((action, i) => {
            // Opening stagger: index 0 appears first (closest to FAB)
            // Closing stagger: last index disappears first
