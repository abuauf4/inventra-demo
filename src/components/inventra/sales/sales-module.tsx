  ...
  return (
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <Button onClick={() => { resetForm(); setCustomerSearch(''); setDialogOpen(true) }} className="bg-gradient-to-r from-rose-500 to-amber-500 text-white"><Plus className="w-4 h-4 mr-2" />Tambah</Button>
      </div>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}><DialogContent className="max-w-2xl"><DialogHeader><DialogTitle>Tambah Penjualan</DialogTitle></DialogHeader>
            ...
            {form.items.map((item, idx) => {
              ...
              return (
                <div key={idx} className="grid grid-cols-12 gap-2 items-center">
                  ...
                  <Input type="number" placeholder="Harga" value={item.sellPrice} onChange={e => updateItem(idx, 'sellPrice', e.target.value)} className="col-span-3 h-8 text-sm"
                    onKeyDown={e => { if (e.key === 'Enter' && item.variantId) { handleSave(); e.preventDefault() } }} />
        ...
        <DialogFooter><Button variant="outline" onClick={() => setDialogOpen(false)}>Batal</Button><Button ref={saveButtonRef} className="bg-gradient-to-r from-rose-500 to-amber-500 text-white" onClick={handleSave}>Simpan</Button></DialogFooter>
