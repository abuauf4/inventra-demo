  ...
  if (selectedId) {
    return (
      <div className="flex items-center gap-1.5 bg-stone-100 dark:bg-white/[0.06] border border-stone-200 dark:border-white/[0.08] rounded-md px-3 py-1.5 h-9 max-w-xs">
        <span className="text-sm text-stone-700 dark:text-stone-300 truncate flex-1">{chipLabel || 'Dipilih'}</span>
        <button
          onClick={handleClear}
          className="text-stone-400 hover:text-stone-700 dark:hover:text-stone-200 shrink-0"
          type="button"
        >
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      {showDropdown && suggestions.length > 0 && (
        <div className="absolute z-50 top-full mt-1 w-64 bg-popover dark:bg-[#1e2130] border rounded-md shadow-lg max-h-60 overflow-auto">
          {suggestions.map(item => (
            <button
              key={item.id}
              type="button"
              className="w-full text-left px-3 py-2 hover:bg-blue-50 dark:hover:bg-blue-900/20 focus:bg-blue-50 dark:focus:bg-blue-900/20 text-sm border-b last:border-b-0"
...
function ProductTypeaheadFilter({
  ...
  return (
    <div className="relative">
      ...
      />
      {showDropdown && suggestions.length > 0 && (
        <div className="absolute z-50 top-full mt-1 w-72 bg-popover dark:bg-[#1e2130] border rounded-md shadow-lg max-h-60 overflow-auto">
          {suggestions.map(item => (
            <button
              key={item.id}
              type="button"
              className="w-full text-left px-3 py-2 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 focus:bg-emerald-50 dark:focus:bg-emerald-900/20 text-sm border-b last:border-b-0"
              onMouseDown={e => { e.preventDefault(); handleSelect(item) }}
            >
              <div className="font-medium text-xs text-muted-foreground">{item.code}</div>
              <div className="text-sm">{item.label}</div>
          ...
          {loading ? <div className="flex justify-center py-8"><RefreshCw className="w-5 h-5 animate-spin text-stone-300" /></div> : (
            <>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
                <Card className="border shadow-sm"><CardContent className="p-3 sm:p-5 flex items-center gap-4">
                  <div className="w-9 h-9 rounded-lg bg-emerald-50 dark:bg-emerald-900/20 flex items-center justify-center"><TrendingUp className="w-4 h-4 text-emerald-600 dark:text-emerald-400" /></div>
                  <div><p className="text-xs text-muted-foreground">Pendapatan</p><p className="text-lg font-bold text-emerald-700 dark:text-emerald-400">{fmtRp(data?.revenue || 0)}</p><p className="text-[10px] text-muted-foreground">COMPLETED + PAID</p></div>
                </CardContent></Card>
                <Card className="border shadow-sm"><CardContent className="p-3 sm:p-5 flex items-center gap-4">
                  <div className="w-10 h-10 rounded-xl bg-rose-100 dark:bg-rose-900/20 flex items-center justify-center"><ShoppingCart className="w-5 h-5 text-rose-600" /></div>
                  <div><p className="text-xs text-muted-foreground">Modal</p><p className="text-lg font-bold text-rose-700 dark:text-rose-400">{fmtRp(data?.totalCost || 0)}</p><p className="text-[10px] text-muted-foreground">Estimasi HPP</p></div>
                </CardContent></Card>
                <Card className="border shadow-sm"><CardContent className="p-3 sm:p-5 flex items-center gap-4">
                  <div className="w-10 h-10 rounded-xl bg-teal-100 dark:bg-teal-900/20 flex items-center justify-center"><TrendingUp className="w-5 h-5 text-teal-600" /></div>
                ...
                <Card className="border shadow-sm"><CardContent className="p-3 sm:p-5 flex items-center gap-4">
                  <div className="w-10 h-10 rounded-xl bg-amber-100 dark:bg-amber-900/20 flex items-center justify-center"><ShoppingCart className="w-5 h-5 text-amber-600" /></div>
              ...
              {/* Period Table */}
              <Card className="border shadow-sm"><CardHeader><CardTitle>Laporan Penjualan per Periode</CardTitle></CardHeader><CardContent>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 sm:gap-4">
                <Card className="border shadow-sm"><CardHeader><CardTitle className="flex items-center gap-2"><Trophy className="w-4 h-4 text-amber-500" />Produk Terlaris (Pendapatan)</CardTitle></CardHeader><CardContent>
                  {data?.topProductsByRevenue?.length > 0 ? <div className="overflow-x-auto -mx-3 sm:mx-0"><Table><TableHeader><TableRow><TableHead>Produk</TableHead><TableHead className="text-center">Qty</TableHead><TableHead className="text-right">Pendapatan</TableHead><TableHead className="text-right">Profit</TableHead></TableRow></TableHeader>
                    <TableBody>{data.topProductsByRevenue.map((p: any, i: number) => <TableRow key={i}><TableCell><div className="font-medium">{p.name}</div><div className="text-xs text-muted-foreground font-mono">{p.sku}</div></TableCell><TableCell className="text-center">{p.qty}</TableCell><TableCell className="text-right font-medium">{fmtRp(p.revenue)}</TableCell><TableCell className="text-right text-teal-600 dark:text-teal-400">{fmtRp(p.revenue - p.cost)}</TableCell></TableRow>)}</TableBody></Table></div> : <p className="text-center text-muted-foreground py-4">Tidak ada data</p>}
                </CardContent></Card>
                <Card className="border shadow-sm"><CardHeader><CardTitle className="flex items-center gap-2"><Users className="w-4 h-4 text-purple-500" />Customer Teratas</CardTitle></CardHeader><CardContent>
          ...
          {loading ? <div className="flex justify-center py-8"><RefreshCw className="w-6 h-6 animate-spin text-stone-300" /></div> : (
            <>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
                <Card className="border shadow-sm"><CardContent className="p-3 sm:p-5 flex items-center gap-4">
                  <div className="w-10 h-10 rounded-xl bg-rose-100 dark:bg-rose-900/20 flex items-center justify-center"><ShoppingCart className="w-5 h-5 text-rose-600" /></div>
                  <div><p className="text-xs text-muted-foreground">Total Biaya (Diterima)</p><p className="text-lg font-bold text-rose-700 dark:text-rose-400">{fmtRp(data?.cost || 0)}</p><p className="text-[10px] text-muted-foreground">Status RECEIVED saja</p></div>
                </CardContent></Card>
                <Card className="border shadow-sm"><CardContent className="p-3 sm:p-5 flex items-center gap-4">
                  <div className="w-10 h-10 rounded-xl bg-amber-100 dark:bg-amber-900/20 flex items-center justify-center"><Package className="w-5 h-5 text-amber-600" /></div>
                  <div><p className="text-xs text-muted-foreground">Total Transaksi</p><p className="text-lg font-bold">{fmt(data?.totalTransactions || 0)}</p><p className="text-[10px] text-muted-foreground">Tanpa DRAFT/CANCELLED</p></div>
                </CardContent></Card>
                <Card className="border shadow-sm"><CardContent className="p-3 sm:p-5 flex items-center gap-4">
                  <div className="w-10 h-10 rounded-xl bg-purple-100 dark:bg-purple-900/20 flex items-center justify-center"><TrendingUp className="w-5 h-5 text-purple-600" /></div>
              ...
              {/* Period Table */}
              <Card className="border shadow-sm"><CardHeader><CardTitle>Laporan Pembelian per Periode</CardTitle></CardHeader><CardContent>
                {data?.grouped?.length > 0 ? <div className="overflow-x-auto -mx-3 sm:mx-0"><Table><TableHeader><TableRow><TableHead>Periode</TableHead><TableHead className="text-center">Jumlah Transaksi</TableHead><TableHead className="text-right">Total</TableHead></TableRow></TableHeader><TableBody>{data.grouped.map((item: any, i: number) => <TableRow key={i}><TableCell>{item.period}</TableCell><TableCell className="text-center">{item.count}</TableCell><TableCell className="text-right font-medium">{fmtRp(item.totalAmount)}</TableCell></TableRow>)}</TableBody></Table></div> : <p className="text-center text-muted-foreground py-8">Tidak ada data</p>}
              </CardContent></Card>

              {/* Top Products + Top Suppliers */}
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 sm:gap-4">
                <Card className="border shadow-sm"><CardHeader><CardTitle className="flex items-center gap-2"><Trophy className="w-4 h-4 text-amber-500" />Produk Termahal (Biaya)</CardTitle></CardHeader><CardContent>
                ...
                <Card className="border shadow-sm"><CardHeader><CardTitle className="flex items-center gap-2"><Truck className="w-4 h-4 text-purple-500" />Supplier Teratas</CardTitle></CardHeader><CardContent>
        ...
        <TabsContent value="stock" className="flex flex-col flex-1 min-h-0 mt-3">
          ...
          {filterLabel && (
            <div className="flex items-center gap-2 flex-wrap shrink-0">
              ...
              <span className="text-xs font-medium text-stone-600 dark:text-stone-300 bg-stone-100 dark:bg-white/[0.06] px-2.5 py-0.5 rounded-full">{filterLabel}</span>
              {filterStockSupplierId && selectedStockSupplier && (
                <span className="text-xs font-medium text-stone-600 dark:text-stone-300 bg-stone-100 dark:bg-white/[0.06] px-2.5 py-0.5 rounded-full">
          ...
          {loading ? <div className="flex justify-center py-8"><RefreshCw className="w-6 h-6 animate-spin text-stone-300" /></div> : (
            <>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
                ...
                <Card className="border shadow-sm"><CardContent className="p-3 sm:p-5 flex items-center gap-4">
                  <div className="w-10 h-10 rounded-xl bg-red-100 dark:bg-red-900/20 flex items-center justify-center"><AlertTriangle className="w-5 h-5 text-red-600" /></div>
                  <div><p className="text-xs text-muted-foreground">Stok Menipis</p><p className="text-lg font-bold text-amber-600 dark:text-amber-400">{fmt(data?.lowStockCount || 0)}</p></div>
                </CardContent></Card>
                <Card className="border shadow-sm"><CardContent className="p-3 sm:p-5 flex items-center gap-4">
                  <div className="w-10 h-10 rounded-xl bg-emerald-100 dark:bg-emerald-900/20 flex items-center justify-center"><TrendingUp className="w-5 h-5 text-emerald-600" /></div>
              ...
              <Card className="border shadow-sm"><CardHeader><CardTitle>Laporan Stok (Per Varian)</CardTitle></CardHeader><CardContent>
                {data?.variants?.length > 0 ? <div className="overflow-x-auto -mx-3 sm:mx-0 max-h-[50vh] sm:max-h-96"><Table><TableHeader><TableRow><TableHead>Produk</TableHead><TableHead>Varian</TableHead><TableHead className="text-center">Stok</TableHead><TableHead className="text-center">Min</TableHead><TableHead className="text-right">Harga Beli</TableHead><TableHead className="text-right">Nilai</TableHead><TableHead>Status</TableHead></TableRow></TableHeader>
                  <TableBody>{data.variants.map((p: any) => <TableRow key={p.id}><TableCell className="font-medium">{p.name || p.productName}</TableCell><TableCell>{p.variantName || '-'}</TableCell><TableCell className="text-center">{p.stock}</TableCell><TableCell className="text-center">{p.minStock}</TableCell><TableCell className="text-right">{fmtRp(p.buyPrice)}</TableCell><TableCell className="text-right">{fmtRp(p.stockValue)}</TableCell>
                    <TableCell><Badge variant={p.stock <= p.minStock ? (p.stock <= 0 ? 'destructive' : 'secondary') : 'default'} className={p.stock > p.minStock ? 'bg-emerald-100 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-400' : p.stock <= 0 ? '' : 'bg-amber-100 dark:bg-amber-900/20 text-amber-700 dark:text-amber-400'}>{p.stock <= 0 ? 'Habis' : p.stock <= p.minStock ? 'Menipis' : 'Aman'}</Badge></TableCell>
        ...
        <TabsContent value="stock-mutations" className="flex flex-col flex-1 min-h-0 mt-3">
          ...
          {loading ? <div className="flex justify-center py-8"><RefreshCw className="w-6 h-6 animate-spin text-stone-300" /></div> : (
            <>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
                <Card className="border shadow-sm"><CardContent className="p-3 sm:p-5 flex items-center gap-4">
                  <div className="w-10 h-10 rounded-xl bg-emerald-100 dark:bg-emerald-900/20 flex items-center justify-center"><ArrowDownCircle className="w-5 h-5 text-emerald-600" /></div>
                  <div><p className="text-xs text-muted-foreground">Stok Masuk (IN)</p><p className="text-lg font-bold text-emerald-700 dark:text-emerald-400">{fmt(data?.byType?.IN?.count || 0)} transaksi</p><p className="text-[10px] text-muted-foreground">Total: {fmt(data?.byType?.IN?.totalQty || 0)} unit</p></div>
                </CardContent></Card>
                <Card className="border shadow-sm"><CardContent className="p-3 sm:p-5 flex items-center gap-4">
                  <div className="w-10 h-10 rounded-xl bg-rose-100 dark:bg-rose-900/20 flex items-center justify-center"><ArrowUpCircle className="w-5 h-5 text-rose-600" /></div>
                  <div><p className="text-xs text-muted-foreground">Stok Keluar (OUT)</p><p className="text-lg font-bold text-rose-700 dark:text-rose-400">{fmt(data?.byType?.OUT?.count || 0)} transaksi</p><p className="text-[10px] text-muted-foreground">Total: {fmt(data?.byType?.OUT?.totalQty || 0)} unit</p></div>
                </CardContent></Card>
                <Card className="border shadow-sm"><CardContent className="p-3 sm:p-5 flex items-center gap-4">
                  <div className="w-10 h-10 rounded-xl bg-amber-100 dark:bg-amber-900/20 flex items-center justify-center"><SlidersHorizontal className="w-5 h-5 text-amber-600" /></div>
                  <div><p className="text-xs text-muted-foreground">Penyesuaian (ADJ)</p><p className="text-lg font-bold text-amber-700 dark:text-amber-400">{fmt(data?.byType?.ADJUSTMENT?.count || 0)} transaksi</p><p className="text-[10px] text-muted-foreground">Total: {fmt(data?.byType?.ADJUSTMENT?.totalQty || 0)} unit</p></div>
                </CardContent></Card>
                <Card className="border shadow-sm"><CardContent className="p-3 sm:p-5 flex items-center gap-4">
                  <div className="w-10 h-10 rounded-xl bg-blue-100 dark:bg-blue-900/20 flex items-center justify-center"><ArrowUpDown className="w-5 h-5 text-blue-600" /></div>
                  <div><p className="text-xs text-muted-foreground">Transfer</p><p className="text-lg font-bold text-blue-700 dark:text-blue-400">{fmt(data?.byType?.TRANSFER?.count || 0)} transaksi</p><p className="text-[10px] text-muted-foreground">Total: {fmt(data?.byType?.TRANSFER?.totalQty || 0)} unit</p></div>
                </CardContent></Card>
              </div>
                            ...
                            <TableRow key={m.id}>
                              ...
                              <TableCell>
                                <Badge variant="outline" className={
                                  m.type === 'IN' ? 'border-emerald-300 dark:border-emerald-700 text-emerald-700 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/20' :
                                  m.type === 'OUT' ? 'border-rose-300 dark:border-rose-700 text-rose-700 dark:text-rose-400 bg-rose-50 dark:bg-rose-900/20' :
                                  m.type === 'ADJUSTMENT' ? 'border-amber-300 dark:border-amber-700 text-amber-700 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/20' :
