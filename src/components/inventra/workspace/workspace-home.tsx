    ...
    <div className="max-w-3xl mx-auto space-y-8">
      ...
      <div className="flex items-center gap-2">
        {quickActions.map((a, i) => (
          <button
            key={i}
            onClick={a.action}
            className="group flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white border border-stone-200/80 hover:border-stone-300 shadow-sm hover:shadow transition-all text-sm font-medium text-stone-700 dark:bg-[#1a1f2e]/80 dark:border-white/[0.06] dark:hover:border-white/[0.10] dark:text-stone-300"
          >
            <div
              className={`w-7 h-7 rounded-lg ${a.color} flex items-center justify-center text-white group-hover:scale-105 transition-transform`}
            >
              {a.icon}
            </div>
            {a.label}
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-2">
            {(data.lowStockProducts ?? []).slice(0, 4).map((p) => (
              <button
                key={p.variantId}
                onClick={() => setActivePage('products')}
                className={`flex items-center gap-2.5 p-3 rounded-xl bg-white border hover:shadow-sm transition-shadow text-left dark:bg-[#1a1f2e]/80 ${
                  p.stock <= 0
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="space-y-3">
          ...
          <div className="bg-white rounded-xl border border-stone-200/80 divide-y divide-stone-100 dark:bg-[#1a1f2e]/80 dark:border-white/[0.06] dark:divide-white/[0.04]">
            {!(data.recentTransactions ?? []).length ? (
              ...
              (data.recentTransactions ?? []).slice(0, 4).map((t, i) => (
                <div
                  key={i}
                  className="flex items-center gap-2.5 px-3.5 py-2.5 hover:bg-stone-50/50 transition-colors dark:hover:bg-white/[0.02]"
                >
                  <div
          ...
          <div className="bg-white rounded-xl border border-stone-200/80 divide-y divide-stone-100 dark:bg-[#1a1f2e]/80 dark:border-white/[0.06] dark:divide-white/[0.04]">
            {inboxItems.length === 0 ? (
              ...
              inboxItems.slice(0, 4).map((item: any) => (
                <div
                  key={item.id}
                  className="flex items-start gap-2.5 px-3.5 py-2.5 hover:bg-stone-50/50 transition-colors dark:hover:bg-white/[0.02]"
                >
                  <div
