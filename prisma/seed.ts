...
async function main() {
  ...
  for (const sup of supplierData) {
    ...
    if (!existing) {
      const created = await db.supplier.create({ data: { code: sup.code, name: sup.name, pic: sup.pic, phone: sup.phone, email: sup.email, address: sup.address } })
  ...
  for (const cust of customerData) {
    ...
    if (!existing) {
      await db.customer.create({ data: { code: cust.code, name: cust.name, phone: cust.phone, email: cust.email || null, address: cust.address || null } })
    }
  }
  ...
  for (const pData of productVariants) {
    ...
    if (!existingProduct) {
      ...
      console.log(`✅ Product: ${pData.name} (${pData.variants.length} variants)`)
    }
  }
  ...
  if (existingInbox === 0) {
    ...
    for (const item of inboxItems) {
      await db.inbox.create({ data: { ...item, entityId: null } })
    }
